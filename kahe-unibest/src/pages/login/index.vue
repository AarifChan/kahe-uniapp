  ...
  <view class="pageLogin">
    ...
    <view class="pageLogin-content">
      <view class="pageLogin-content-top">
        <view class="pageLogin-content-top-title theme-font">
          欢迎登录
        </view>
        <image
...
async function handleSendSms() {
  ...
  if (!validatePhone(phone))
    return
  if (smsSending.value || smsCountdown.value > 0)
    return
...
async function handleSubmit() {
  ...
  const err = await userStore.handleLoginByPhone({
    phone,
    password: credential,
    type: isSms ? 0 : 1,
  })
...
async function handleWechatOneClick() {
  if (!validateAgreement())
    return
  ...
  const handleCodeLogin = async (code?: string) => {
    ...
    const err = await userStore.handleAppWechatLogin({ code })
    if (err) {
      ShowToast(`${err}`)
    }
    else {

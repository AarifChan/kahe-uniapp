  ...
  <view class="homeGoods">
    ...
    <view class="homeGoods-cover">
      ...
      <view class="homeGoods-cover-pattern">
        <image
          v-for="(label, index) in item.labels" :key="`labels:${index}`" :src="formatLabelImage(label)"
    ...
    <view class="homeGoods-info">
      <view class="homeGoods-info-content">
        <view class="homeGoods-info-content-bottom">
          <view class="homeGoods-info-content-bottom-title text-flow-ellipsis-single">
            {{ item.name }}
          </view>
          <view class="homeGoods-info-content-bottom-row1">
            <view class="homeGoods-info-content-bottom-row1-price">
              ...
              />
              <text class="homeGoods-info-content-bottom-row1-price-title price-font">
                {{ isFromMall ? ""
                  : "" }}{{ item.price
                }}{{ isFromMall ? "/点券" : "" }}
              </text>
        ...
        <view v-if="!hideMerchant" class="homeGoods-info-content-merchant">
          <image class="homeGoods-info-content-merchant-logo" :src="item.merchant?.icon" />
          <view class="homeGoods-info-content-merchant-name text-flow-ellipsis-single">
            {{ item.merchant?.name
            }}
          </view>
        </view>
...
function formatLabelName(index: number) {
  switch (index) {
    case 1:
      return '满开'
    case 2:

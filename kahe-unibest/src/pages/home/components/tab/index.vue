  ...
  <view class="homeTab">
    <scroll-view class="homeTab-list" :enable-flex="true" :scroll-x="true" scroll-with-animation>
      <view class="homeTab-content">
        <view v-for="(item, index) in list" :key="index" class="homeTab-list-item" @tap.stop="clickItem(index)">
          <view v-if="current === index" class="homeTab-list-item-select theme-font">
            {{ item.title }}
            <image src="https://jms.85gui7.com/kahe-202510/ka-he/home/round.png" class="img" />
          </view>

          <view v-else class="homeTab-list-item-normal theme-font">
            {{ item.title }}
          </view>
        </view>
      </view>

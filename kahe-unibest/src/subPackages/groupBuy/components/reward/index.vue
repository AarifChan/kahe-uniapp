<template>
  <view class="groupBuyReward">
    ...
    <view v-if="!opened" class="groupBuyReward-content">
    ...
    <view v-else class="groupBuyReward-content">
      ...
      <view class="groupBuyReward-content-center">
        <z-swiper v-model="cardList">
          ...
          >
            <view class="groupBuyReward-content-center-title">
              #{{ item.sid }}-{{ item.name }} {{ item.sname }} {{ getNormalLevelNameByLevel(item.level) }}
            </view>
      ...
      <view class="groupBuyReward-content-tips">
        卡密以原版信息为准，中文翻译及图片【仅供参考】
      </view>
...
import type { GroupBuyCard } from '@/model'

  ...
  <view class="groupBuyIndex">
    ...
    <view class="groupBuyIndex-content">
      <view class="groupBuyIndex-content-top">
        <search placeholder="请输入你想要搜索的内容" style="flex: 1;" @did-tap-search="handleSearch" />
        <view class="rule theme-font" @tap.stop="showModalType(ModalType.GroupBuyRule)">
          规 则
        </view>
      </view>
      <Handle @did-change="handleSortItem" />
      <scroll-view class="groupBuyIndex-content-scroll" scroll-y @scrolltolower="scrollToLower">
        <view class="groupBuyIndex-content-list">
          <Item
            v-for="(item, index) in groupBuyList" :key="`item${index}`" :item="item"
...
function handleSortItem(params: { sort: string, sflag: number }) {

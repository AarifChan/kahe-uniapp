  ...
  <view class="card">
    <view class="bottoms">
      ...
      <view class="content">
        ...
        <text class="content-title theme-font">
          单人{{ (productDetail.another / 5).toFixed(0)
          }}包,必出高罕卡
        </text>
      </view>
      <view class="btnList">
        <view
          v-for="(item, index) in list" :key="index" class="btnList-bolist"
          @tap.stop="$emit('tapCards', item)"
        >
          <image src="https://jms.85gui7.com/kahe-202510/ka-he/machine/card-btn.png" class="img" />
          <view class="num theme-font">
            {{ item.num / 5 }}包
          </view>
        </view>
      </view>

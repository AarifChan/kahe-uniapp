<template>
  <view class="infinite">
    <view class="infinite-top">
      <image
        src="https://jms.85gui7.com/kahe-202510/ka-he/product/infinite-top-bg.png"
        class="infinite-top-img"
      />
      <view class="infinite-top-btn">
        <view class="infinite-top-btn-item style1 theme-font" @tap.stop="tapShowModel(0)">
          发货须知
        </view>
        <view class="infinite-top-btn-item style2 theme-font" @tap.stop="tapShowModel(1)">
          购买说明
        </view>
        <view class="infinite-top-btn-item style3 theme-font" @tap.stop="tapShowModel(2)">
          品相定义
        </view>
      </view>
      <!--      <view -->
    ...
    <view class="infinite-top-swiper">
      <z-swiper v-model="bannerList" :options="options">
        <z-swiper-item v-for="(item, index) in bannerList" :key="index" :custom-style="slideCustomStyle">
          <view class="imageBox">
    ...
    <view v-else class="infinite-center">
      <view class="boxItem">
        ...
        <view class="infinite-center-right" @tap.stop="openChestModal(1)">
          <image
            class="infinite-center-right-img"
            src="https://jms.85gui7.com/kahe-202510/ka-he/product/chest.png"
          />
          <view class="infinite-center-right-title theme-font">
            查看宝箱
          </view>
        </view>
        <view class="infinite-center-left">
          <view class="infinite-center-left-title">
            再抽{{ product.another }}发必出{{ product.nextLevel }}以上<text
              class="infinite-center-left-title-tips"
            >
              (若提前出了则幸运值重置)
            </text>
          </view>
          <view class="infinite-center-left-progressBg">
            <view class="infinite-center-left-progressBg-progress" :style="luckProgress" />
          </view>
        </view>
      </view>
    </view>
    <view v-if="product?.attachList?.length > 0" class="infinite-tips" @tap.stop="openChestModal(0)">
      <image class="infinite-tips-bg" src="https://jms.85gui7.com/kahe-202510/ka-he/product/decorate.png" />
      <view class="infinite-tips-title theme-font">
        隐藏掉落
      </view>
      <view class="infinite-tips-tips">
        每次购买均有独立概率触发掉落，存在一次多得的情况
      </view>
    </view>
    <InGroup :content="product.remark" @tap.stop="emits('didClickRemark')" />
    <CommonTab
      v-model:current-index="currentIndex" :product="product" class="infinite-common"
      @scroll-to-lower="scrollToLower" @tab-did-change="tabDidChange" @did-click-button="handleClickAction"
    >
      <template #goods>
        <in-goods
          v-for="(item, index) in levelGroupList" :key="index" :product="product" :item="item"
      ...
      <template #record>
        <view>
          ...
          <record
            :record-list="recordList" :types="true" :is-infinite="true" :border-heigth="201"
            :border-width="701"
          />
        </view>
      </template>
    ...
    <cards
      :cards-array="cardsArray" :is-infinite="true" :queue-lock-time="product.queueLockTime"
      @did-tap-choose="didClickPurchase"
    />
  </view>
  <chest-modal
    v-model:show="chestModalShow" :type="chestModalType" :price="product.price" :goods-list="chestList"
    :product="product"
  />
</template>
...
function tapShowModel(index: number) {
  emits('tapShowModel', index)
}

const bannerList = ref([] as any)
function openChestModal(type: number) {

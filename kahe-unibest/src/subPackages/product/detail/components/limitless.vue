...
import type { LimitLessModel } from './limitlessItem.vue'
import type { UIProductBoxModel, UIProductDetailLevelList, UIProductDetailModel, UIProductPriceCard, UIProductRecordModel } from '@/model'
import { computed, ref } from 'vue'
import { limitLevelName } from '@/utils/tools/image'
import Item from './limitlessItem.vue'
...
function handleTapTab(index: number) {
  currentTab.value = index
  emits('tabDidChange', index)
}
function changeIndex(isLeft: boolean) {
  ...
  &-bottom {
    ...
    &-content {
      ...
      &-record {
        ...
        width: calc(100% - 100rpx);
        display: grid;
        grid-template-columns: repeat(auto-fill, minmax(48%, 1fr)); // 这里的100px是假设的最小宽度，1fr是灵活的宽度
        ...
        &-item {
          ...
          border-radius: 4rpx;
          background: linear-gradient(0deg, rgba(85, 99, 169, 0.99), rgba(85, 99, 169, 0.01));
          display: flex;
          flex-direction: row;
          align-items: center;

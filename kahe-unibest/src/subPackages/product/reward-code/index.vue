<template>
  <view class="code">
    <view class="code-tab">
      <view
        v-for="(item, index) in tabList" :key="index" class="code-tab-tabList" :class="current === item.value ? 'active' : ''"
    ...
    <scroll-view class="code-list" :scroll-y="true" :enable-flex="true">
      <view v-for="(item, index) in codeList" :key="index" class="code-list-item">
        <view class="code-list-item-left">
          <view class="code-list-item-left-code">
            {{ calculateMD5(item.id, 10) }}
          </view>
          <view class="code-list-item-left-time">
            {{ item.time }}初始获得
          </view>
          <view class="code-list-item-left-title">
            福利抽选
          </view>
...
const current = ref(0)
async function tabActive(value: number) {
  current.value = value

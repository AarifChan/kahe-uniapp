  ...
  <view class="vip">
    ...
    <view class="vip-top">
      <view class="vip-top-content">
        ...
        <view class="vip-top-content-rightTitle" @tap.stop="handleShowRule">
          ...
          <text
            class="vip-top-content-rightTitle-title" style="font-size: 16rpx;
        ...
        <view class="vip-top-content-info">
          <view class="vip-top-content-info-level">
            ...
            <view class="vip-top-content-info-level-progress">
              <view class="vip-top-content-info-level-progress-value" :style="progressStyle" />
              <text class="vip-top-content-info-level-progress-title text-flow-ellipsis-single">
                今日成长值{{
                  userInfo.expDay ?? 0 }}点
              </text>
            </view>
          ...
          <view class="vip-top-content-info-current">
            <view style="margin-right: 50rpx; display: flex;">
              <view class="vip-top-content-info-current-title">
                当前
              </view>
              <view class="vip-top-content-info-current-value">
                {{ userInfo.exp ?? 0 }}
              </view>
              <view class="vip-top-content-info-current-title">
                点，需要{{ currentLevelExp }}点升级
              </view>
            </view>
          </view>
        </view>
    ...
    <view class="vip-table">
      <view class="vip-table-head">
        ...
        <view class="vip-table-head-title theme-font" style="color: #fff;">
          等级
        </view>
        <view class="vip-table-head-title theme-font">
          升级奖励
        </view>
        <view class="vip-table-head-title theme-font">
          每日奖励
      ...
      <scroll-view class="vip-table-content" :enable-flex="true" :scroll-y="true" scroll-with-animation>
        <view
          v-for="(item, index) in vipModels" :id="`vip-table:id${index}`" :key="`vip-table:key${index}`"
          class="vip-table-content-row"
        >
          <image
            class="vip-table-content-row-bg"
          ...
          <view class="vip-table-content-row-item vip-table-content-row-column">
            <view
              v-for="(v, vIndex) in item.rewards" :id="`${index}reward:id${vIndex}`"
              :key="`${index}reward:key${vIndex}`" class="vip-table-content-row-item-c"
            >
              <image
                v-if="v.name === '点券'" src="https://jms.85gui7.com/kahe-202510/images/vip-icon1.png" style="height: 27rpx;"
                mode="heightFix"
              />
              <image v-if="v.logo" :src="v.logo" style="height: 27rpx;" mode="heightFix" />
              <text class="vip-table-content-row-item-c-title">{{ v.name }} x{{ v.num }}</text>
            </view>
          </view>
          <view class="vip-table-content-row-item vip-table-content-row-column">
            <view
              v-for="(v, vIndex) in item.rewardsDay" :id="`${index}rewardsDay:id${vIndex}`"
            ...
            >
              <image
                ...
                mode="heightFix"
              />
              <text class="vip-table-content-row-item-d-title">
                {{ v.num ? v.num + v.name : v.name
                }}
              </text>
            </view>
          </view>
    ...
    <view class="page-control">
      <image
        class="page-control-arrow" src="https://jms.85gui7.com/kahe/vip/left.png" mode="aspectFit"
        @tap.stop="changePage(false)"
      />
      <view
        v-for="(item, index) in pageItem" :id="`vip-pageControl:id${index}`" :key="`vip-pageControl:${index}`"
        class="page-control-item" :class="item === pageParams.page ? 'active' : ''"
        @tap.stop="changeCurrentPage(item)"
      >
        {{ item }}
      </view>
      <image
        class="page-control-arrow" src="https://jms.85gui7.com/kahe/vip/right.png" mode="aspectFit"
...
const currentLevelExp = ref(
  computed(() => {
    let levelExp = 0
    for (const item of userStore.vipLevelsModel) {
...
const vipModels = computed((): VipsLevelModel[] => userStore.vipLevelsModel)
...
function changePage(isForward: boolean) {
...
async function loadVipLevelData() {
  const totalNum = await userStore.getVipLevelList(pageParams.value)

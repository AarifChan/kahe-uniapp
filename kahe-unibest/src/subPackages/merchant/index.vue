<template>
  <scroll-view class="merchantInfo" :scroll-y="true" @scrolltolower="handleScrollToLower">
    ...
    <view class="merchantInfo-content">
      <view class="welfare">
        ...
        <view class="merchantInfo-content-top">
          ...
          <view class="merchantInfo-content-top-info">
            ...
            <view
              class="merchantInfo-content-top-info-bar" :style="{
            ...
            <view class="merchantInfo-content-top-info-desc text-flow-ellipsis-multiple">
              {{ detail?.desc }}
            </view>
          </view>
      ...
      <view class="bottom">
        <tab
          v-model:current="current" :list="merchantTabList" :search="true"
          @did-click="didClickTab(merchantTabList[current].value)"
        />
        <view class="merchantInfo-content-padding">
          <view
            v-for="(item, index) in goodsList" :key="index" class="merchantInfo-content-padding-item"
...
function didClickScore() {
  // appStore.changeProductTabIndex(3)
  uni.navigateTo({
...
function handleClickShare() {
  ...
  shareWeixinMiniProgramCard({
    title: `${userStore.userInfo?.nickname ?? ''}邀请你来卡核抽取各种稀有卡牌！`,

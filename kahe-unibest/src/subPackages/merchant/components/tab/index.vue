<template>
  <view class="merchantTab">
    <view class="merchantTab-list">
      <view v-for="(item, index) in list" :key="index" class="merchantTab-list-item" @tap.stop="clickItem(index)">
        <view class="merchantTab-list-item-content">
          <view v-if="current === index" class="merchantTab-list-item-content-select theme-font">
            <image src="https://jms.85gui7.com/kahe-202510/ka-he/home/round.png" class="img" />
            {{ item.title }}
          </view>
          <text v-else class="merchantTab-list-item-content-title theme-font">{{ item.title }}</text>
        </view>
      </view>
    </view>
  </view>

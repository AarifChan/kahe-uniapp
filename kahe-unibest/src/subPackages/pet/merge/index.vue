<template>
  <view class="merge">
    ...
    <view class="merge-bottom">
      ...
      <scroll-view
        class="merge-bottom-scroll" :scroll-y="true" :enable-flex="true" scroll-with-animation
        @scrolltolower="handleMinePetScrollToLower"
      >
        <view class="merge-bottom-scroll-content">
          <pet
            v-for="(item, index) in minePetList" :key="`pet${index}`" :item="item"
...
function handleClickItem(item: PetModel) {
  if (item.level === 4) {

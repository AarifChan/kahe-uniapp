<template>
  <view class="eggs">
    ...
    <view class="eggs-bottom">
      <scroll-view
        class="eggs-bottom-scroll" :scroll-x="false" :scroll-y="true" :enable-flex="true"
        scroll-with-animation
        @tap.stop="currentEgg = null"
      >
        <view class="eggs-bottom-scroll-content">
          <egg
            v-for="(item, index) in mineEggsList" :key="`egg${index}`" :item="item" :selected="currentEgg?.id === item.id"

<template>
  <view class="exploration">
    ...
    <view class="exploration-bottom">
      ...
      <scroll-view
        class="exploration-bottom-scroll" :scroll-y="true" :enable-flex="true" scroll-with-animation
        @scrolltolower="handleMinePetScrollToLower"
      >
        <view class="exploration-bottom-scroll-content">
          <pet
            v-for="(item, index) in minePetList" :key="`pet${index}`" :item="item" :selected="currentPet?.id === item.id"
...
function handleSelectPet(item: PetModel) {

    ...
    <scroll-view class="gift-scroll" @scrolltolower="scrollToLower">
      <view class="gift-scroll-content">
        ...
        <view class="gift-scroll-content-list">
          <view v-for="(item, index) in list" :id="`id${index}`" :key="`i${index}`" class="gift-scroll-content-list-item">
            <view class="gift-scroll-content-list-item-top">
              ...
              <view class="gift-scroll-content-list-item-top-info">
                <view class="gift-scroll-content-list-item-top-title theme-font">
                  {{ item.name }}
                </view>
                <view v-if="lastDay(item.expireTime) > 0" class="gift-scroll-content-list-item-top-subTitle">
                  礼包待领天数：{{ lastDay(item.expireTime) }}
                </view>
              </view>
            </view>
            <view class="gift-scroll-content-list-item-bottom">
              <view v-for="(zItem, zIndex) in item.rewards" :id="`${index}:${zIndex}`" :key="`${index}:${zIndex}`" class="gift-scroll-content-list-item-bottom-item">
                ...
                <view class="gift-scroll-content-list-item-bottom-item-info">
                  <view class="gift-scroll-content-list-item-bottom-item-info-title">
                    购买必得
                  </view>
                  <view class="gift-scroll-content-list-item-bottom-item-info-subTitle">
                    {{ zItem.num }}{{ zItem.name }}
                  </view>
                </view>
              </view>
              <view v-for="(zItem, zIndex) in item.rewardsDay" :id="`${index}:${zIndex}`" :key="`${index}:${zIndex}`" class="gift-scroll-content-list-item-bottom-item">

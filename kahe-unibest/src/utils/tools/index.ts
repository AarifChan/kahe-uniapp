...
export function useSortFun(list: UIProductBoxModel[]): UIProductBoxModel[] {
  list = list.sort((n1, n2) => n2.level - n1.level)
  let finalArr = new Array<UIProductBoxModel>()
  let firstArr = new Array<UIProductBoxModel>()
  let lastArr = new Array<UIProductBoxModel>()
  const totalArr = new Array<UIProductBoxModel>()
  list.forEach((item) => {
    if (firstLevel.includes(item.level)) {
      firstArr.push(item)
    }
    else if (lastLevel.includes(item.level)) {
      lastArr.push(item)
    }
    else if (
      item.level === 205
...
export function getPageOptions(): any {
...
export function handleMobile(phone: string) {
  if (phone) {
    const pat = /(\d{3})\d*(\d{4})/

<template>
  <view class="w-full h-screen bg-[#f5f5f5] flex flex-col">
    <view class="flex flex-row bg-white border-b border-[#e5e5e5]">
      <view
        class="flex-1 h-88 leading-88 text-center text-28 text-[#666] relative"
        :class="{ 'text-[#775435] font-bold': currentTab === 1 }"
        @click="currentTab = 1"
      >
        用户服务协议
        <view v-if="currentTab === 1" class="absolute bottom-0 left-1/2 -translate-x-1/2 w-60 h-4 rounded-2 bg-[#775435]"></view>
      </view>
      <view
        class="flex-1 h-88 leading-88 text-center text-28 text-[#666] relative"
        :class="{ 'text-[#775435] font-bold': currentTab === 2 }"
        @click="currentTab = 2"
      >
        隐私政策
        <view v-if="currentTab === 2" class="absolute bottom-0 left-1/2 -translate-x-1/2 w-60 h-4 rounded-2 bg-[#775435]"></view>
      </view>
    </view>
    <scroll-view
      ...
      @scroll="handleScroll"
    >
      <view class="p-24 bg-white min-h-full">
...
<div style="font-size: 14px; line-height: 1.8; color: #333;">
  ...
  <p style="margin-bottom: 8px; padding-left: 12px;"><strong>1.1 您在使用我们服务时主动提供的信息</strong></p>

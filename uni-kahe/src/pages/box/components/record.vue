<template>
  <view
    class="w-full h-344 relative bg-white border-2 border-[#0E050A] rounded-12 mb-16"
  >
    <view
      class="absolute left-36 top-32 flex flex-col"
      style="width: calc(100% - 50rpx); height: 100%"
    >
        ...
        >
          <view class="relative flex flex-col">
            <view class="w-152 h-152 relative">
              <image
                class="absolute left-0 top-0 w-144 h-144 border-4 border-[#151714] rounded-4"
                mode="aspectFit"
                :src="gItem.image"
              />
              ...
              />
              <text
                class="absolute right-4 bottom-4 text-24 w-70 h-30 text-center font-normal text-white bg-black rounded-[27rpx_0rpx_10rpx_2rpx]"
                >x{{ gItem?.num ?? 1 }}</text
              >
            </view>
    ...
    <view
      class="absolute right-8 top-8 w-141 h-40 bg-[#FFB500] flex flex-row items-center justify-center"
    >
      <!--      <image-->
      <!--        class="absolute right-0 top-0 w-full h-full"-->
      <!--        src="/static/kahe-202510/images/num-bg.png"-->
      <!--      />-->
      <text class="text-24 font-normal text-[#151714] text-center"
        >共{{ total }}件</text
      >
    </view>
  </view>
...
const props = defineProps({
  item: {
    default: {} as UIMineBoxRecordModel,
